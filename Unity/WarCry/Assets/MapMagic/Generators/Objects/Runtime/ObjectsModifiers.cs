...
namespace MapMagic.Nodes.ObjectsGenerators
{
	...
	[System.Serializable]
	[GeneratorMenu(menu = "Objects/Modifiers", name = "Transform",
	    iconName = "GeneratorIcons/Direction", disengageable = true)]
	public class ObjectsTransform210 : Generator, IInlet<TransitionsList>, IOutlet<TransitionsList>
	{
	    public override (string, int) GetCodeFileLine() =>
	        GetCodeFileLineBase(); //to get here with right-click on generator
	    ...
	    public override void Generate(TileData data, StopToken stop)
	    {
	        ...
	        // Creating result list with same objects as src
	        TransitionsList dst = new TransitionsList(src);

	        // Apply transformation based on selected type
	        ApplyTransformation(dst, transformationType, data.area.full.worldSize, data.area.full.worldPos);

	        // Store result
	        data.StoreProduct(this, dst);
	    }
	    
	    private void ApplyTransformation(TransitionsList transitions, TransformationType transformation, 
	        Vector2D worldSize, Vector2D worldPos)
	    {
	        ...
	        Vector2D center = new Vector2D(
	            ...
	            worldPos.z + worldSize.z * 0.5f);

	        for (int i = 0; i < transitions.count; i++)
	        {
	            Transition trs = transitions.arr[i];
	            
	            // Calculate relative position from center
	            float relX = trs.pos.x - center.x;
	            float relZ = trs.pos.z - center.z;

	            // Transform the position based on the selected transformation type
	            switch (transformation)
	            {
	                case TransformationType.FlipX:
	                    // Flip horizontally (mirror across vertical axis)
	                    trs.pos.x = center.x - relX;
	                    // Also need to flip the rotation
	                    trs.Yaw = 180 - trs.Yaw;
	                    break;

	                case TransformationType.FlipZ:
	                    // Flip vertically (mirror across horizontal axis)
	                    trs.pos.z = center.z - relZ;
	                    // Also need to flip the rotation
	                    trs.Yaw = -trs.Yaw;
	                    break;

	                case TransformationType.Transpose:
	                    // Transpose (swap x and z coordinates)
	                    trs.pos.x = center.x + relZ;
	                    trs.pos.z = center.z + relX;
	                    // Adjust rotation - 90° CW if positive, -90° if negative
	                    trs.Yaw = 90 - trs.Yaw;
	                    break;
	            ...
	            transitions.arr[i] = trs;
	        }
	    }
	}

	[System.Serializable]
	[GeneratorMenu(menu = "Objects/Modifiers", name = "Symmetrize",
	    iconName = "GeneratorIcons/Direction", disengageable = true)]
	public class ObjectsSymmetrize210 : Generator, IInlet<TransitionsList>, IOutlet<TransitionsList>
	{
	    public override (string, int) GetCodeFileLine() =>
	        GetCodeFileLineBase(); //to get here with right-click on generator
	    ...
	    {
	        None,
	        HorizontalMidpoint,  // x-axis symmetry
	        VerticalMidpoint,    // z-axis symmetry
	        DiagonalZEqualsX,    // z=x line symmetry
	        DiagonalZEqualsNegativeX, // z=-x line symmetry
	        CenterPoint,         // Combined diagonals - both z=x and z=-x
	        Quadrant,            // Four-fold symmetry
	        Octant               // Eight-fold symmetry
	    }

	    [Val("Symmetry Type")]
	    public SymmetryType symmetryType = SymmetryType.None;
	    
	    [Val("Keep Original", "Keep the original objects in addition to symmetrized copies")]
	    ...
	    public override void Generate(TileData data, StopToken stop)
	    {
	        ...
	        if (!enabled || symmetryType == SymmetryType.None)
	        {
	            data.StoreProduct(this, src);
	            return;
	        }
	        ...
	        // Create a new transitions list (start with empty one, we'll add objects as needed)
	        TransitionsList dst = new TransitionsList();
	        
	        // If keeping originals, add them to the output
	        if (keepOriginal)
	        {
	            dst.Add(src);
	        }
	        
	        // Calculate world bounds and center
	        Vector2D center = new Vector2D(
	            data.area.full.worldPos.x + data.area.full.worldSize.x * 0.5f,
	            data.area.full.worldPos.z + data.area.full.worldSize.z * 0.5f);
	            
	        // Apply symmetry based on selected type
	        switch (symmetryType)
	        {
	            case SymmetryType.HorizontalMidpoint:
	                AddHorizontalSymmetry(src, dst, center);
	                break;
	                
	            case SymmetryType.VerticalMidpoint:
	                AddVerticalSymmetry(src, dst, center);
	                break;
	                
	            case SymmetryType.DiagonalZEqualsX:
	                AddDiagonalZXSymmetry(src, dst, center);
	                break;
	                
	            case SymmetryType.DiagonalZEqualsNegativeX:
	                AddDiagonalZNegativeXSymmetry(src, dst, center);
	                break;
	                
	            case SymmetryType.CenterPoint:
	                AddCenterPointSymmetry(src, dst, center);
	                break;
	                
	            case SymmetryType.Quadrant:
	                AddQuadrantSymmetry(src, dst, center);
	                break;
	                
	            case SymmetryType.Octant:
	                AddOctantSymmetry(src, dst, center);
	                break;
	        }
	    ...
	    private void AddHorizontalSymmetry(TransitionsList src, TransitionsList dst, Vector2D center)
	    {
	        // Mirror each object horizontally
	        for (int i = 0; i < src.count; i++)
	        {
	            Transition trs = src.arr[i];
	            
	            // Skip objects that are already on the symmetry line to avoid duplicates
	            if (Mathf.Approximately(trs.pos.x, center.x))
	                continue;
	                
	            // Create mirrored object
	            Transition mirror = trs;
	            
	            // Calculate relative position and mirror it
	            float relX = trs.pos.x - center.x;
	            mirror.pos.x = center.x - relX;
	            
	            // Mirror the rotation as well
	            mirror.Yaw = 180 - trs.Yaw;
	            
	            // Add mirrored object with a new hash for randomization
	            mirror.hash = trs.hash + 10000; // offset hash to ensure different random values
	            dst.Add(mirror);
	        }
	    }
	    
	    private void AddVerticalSymmetry(TransitionsList src, TransitionsList dst, Vector2D center)
	    {
	        // Mirror each object vertically
	        for (int i = 0; i < src.count; i++)
	        {
	            Transition trs = src.arr[i];
	            
	            // Skip objects that are already on the symmetry line to avoid duplicates
	            if (Mathf.Approximately(trs.pos.z, center.z))
	                continue;
	            ...
	            // Calculate relative position and mirror it
	            float relZ = trs.pos.z - center.z;
	            mirror.pos.z = center.z - relZ;
	            
	            // Mirror the rotation as well
	            mirror.Yaw = -trs.Yaw;
	            
	            // Add mirrored object with a new hash for randomization
	            mirror.hash = trs.hash + 20000;
	            dst.Add(mirror);
	        }
	    }
	    
	    private void AddDiagonalZXSymmetry(TransitionsList src, TransitionsList dst, Vector2D center)
	    {
	        // Mirror each object across z=x diagonal
	        for (int i = 0; i < src.count; i++)
	        {
	            Transition trs = src.arr[i];
	            ...
	            // Skip objects that are already on the symmetry line to avoid duplicates
	            if (Mathf.Approximately(relX, relZ))
	                continue;
	                
	            // Create mirrored object
	            Transition mirror = trs;
	            
	            // Swap x and z coordinates (relative to center)
	            mirror.pos.x = center.x + relZ;
	            mirror.pos.z = center.z + relX;
	            
	            // Adjust rotation - 90° CW if positive, -90° if negative
	            mirror.Yaw = 90 - trs.Yaw;
	            
	            // Add mirrored object with a new hash for randomization
	            mirror.hash = trs.hash + 30000;
	            dst.Add(mirror);
	        }
	    }
	    
	    private void AddDiagonalZNegativeXSymmetry(TransitionsList src, TransitionsList dst, Vector2D center)
	    {
	        // Mirror each object across z=-x diagonal
	        for (int i = 0; i < src.count; i++)
	        {
	            ...
	            // Mirror across z=-x diagonal (relative to center)
	            mirror.pos.x = center.x - relZ;
	            mirror.pos.z = center.z - relX;
	            ...
	            // Add mirrored object with a new hash for randomization
	            mirror.hash = trs.hash + 40000;
	            dst.Add(mirror);
	        }
	    }
	    
	    private void AddCenterPointSymmetry(TransitionsList src, TransitionsList dst, Vector2D center)
	    {
	        // Mirror each object through the center point (180° rotation)
	        for (int i = 0; i < src.count; i++)
	        {
	            Transition trs = src.arr[i];
	            
	            // Skip objects that are already at the center to avoid duplicates
	            if (Mathf.Approximately(trs.pos.x, center.x) && Mathf.Approximately(trs.pos.z, center.z))
	                continue;
	                
	            // Create mirrored object
	            Transition mirror = trs;
	            
	            // Calculate relative position and mirror it through center
	            float relX = trs.pos.x - center.x;
	            float relZ = trs.pos.z - center.z;
	            mirror.pos.x = center.x - relX;
	            mirror.pos.z = center.z - relZ;
	            ...
	            // Rotate 180 degrees
	            mirror.Yaw = trs.Yaw + 180;
	            
	            // Add mirrored object with a new hash for randomization
	            mirror.hash = trs.hash + 50000;
	            dst.Add(mirror);
	        }
	    }
	    
	    private void AddQuadrantSymmetry(TransitionsList src, TransitionsList dst, Vector2D center)
	    {
	        // Create a mirror in each of the four quadrants
	        for (int i = 0; i < src.count; i++)
	        {
	            Transition trs = src.arr[i];
	            ...
	            // Create the object in each quadrant
	            // Quadrant 2 (top-right)
	            if (relX < 0 || relZ < 0) // If not already in Q2
	            {
	                Transition q2 = trs;
	                q2.pos.x = center.x + Mathf.Abs(relX);
	                q2.pos.z = center.z + Mathf.Abs(relZ);
	                
	                // Adjust rotation based on which quadrant the source is in
	                if (relX < 0 && relZ >= 0) // Q1
	                    q2.Yaw = 180 - trs.Yaw;
	                else if (relX < 0 && relZ < 0) // Q3
	                    q2.Yaw = trs.Yaw + 180;
	                else if (relX >= 0 && relZ < 0) // Q4
	                    q2.Yaw = -trs.Yaw;
	                
	                q2.hash = trs.hash + 60000;
	                dst.Add(q2);
	            }
	            
	            // Quadrant 1 (top-left)
	            if (relX >= 0 || relZ < 0) // If not already in Q1
	            {
	                Transition q1 = trs;
	                q1.pos.x = center.x - Mathf.Abs(relX);
	                q1.pos.z = center.z + Mathf.Abs(relZ);
	                ...
	                q1.hash = trs.hash + 70000;
	                dst.Add(q1);
	            }
	            
	            // Quadrant 3 (bottom-left)
	            if (relX >= 0 || relZ >= 0) // If not already in Q3
	            {
	                Transition q3 = trs;
	                q3.pos.x = center.x - Mathf.Abs(relX);
	                q3.pos.z = center.z - Mathf.Abs(relZ);
	                
	                // Adjust rotation based on which quadrant the source is in
	                if (relX >= 0 && relZ >= 0) // Q2
	                    q3.Yaw = trs.Yaw + 180;
	                else if (relX < 0 && relZ >= 0) // Q1
	                    q3.Yaw = -trs.Yaw;
	                else if (relX >= 0 && relZ < 0) // Q4
	                    q3.Yaw = 180 - trs.Yaw;
	                
	                q3.hash = trs.hash + 80000;
	                dst.Add(q3);
	            }
	            
	            // Quadrant 4 (bottom-right)
	            if (relX < 0 || relZ >= 0) // If not already in Q4
	            {
	                Transition q4 = trs;
	                q4.pos.x = center.x + Mathf.Abs(relX);
	                q4.pos.z = center.z - Mathf.Abs(relZ);
	                ...
	                q4.hash = trs.hash + 90000;
	                dst.Add(q4);
	            }
	        }
	    }
	    
	    private void AddOctantSymmetry(TransitionsList src, TransitionsList dst, Vector2D center)
	    {
	        // First apply quadrant symmetry
	        TransitionsList quadrantList = new TransitionsList();
	        if (keepOriginal)
	            quadrantList.Add(src);
	        AddQuadrantSymmetry(src, quadrantList, center);
	        
	        // Then apply diagonal symmetry to all objects
	        for (int i = 0; i < quadrantList.count; i++)
	        {
	            Transition trs = quadrantList.arr[i];
	            
	            // Calculate relative position
	            float relX = trs.pos.x - center.x;
	            float relZ = trs.pos.z - center.z;
	            
	            // Check if object is in upper-right portion of its quadrant
	            if (Mathf.Abs(relX) > Mathf.Abs(relZ))
	            {
	                // Create mirror across z=x or z=-x diagonal as appropriate
	                Transition mirror = trs;
	                
	                // Determine which diagonal to mirror across based on which octant we're in
	                if ((relX > 0 && relZ > 0) || (relX < 0 && relZ < 0)) // Q2 or Q3
	                {
	                    // Mirror across z=x diagonal
	                    mirror.pos.x = center.x + relZ;
	                    mirror.pos.z = center.z + relX;
	                    mirror.Yaw = 90 - trs.Yaw;
	                }
