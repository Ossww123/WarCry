...
namespace MapMagic.Nodes.MatrixGenerators
{
	...
	*/
    ...
    [System.Serializable]
    [GeneratorMenu(menu = "Map/Modifiers", name = "Transform",
        iconName = "GeneratorIcons/Direction", disengageable = true)]
    public class Transform210 : Generator, IInlet<MatrixWorld>, IOutlet<MatrixWorld>
    {
        ...
        public override void Generate(TileData data, StopToken stop)
        {
            ...
            // Creating result matrix with same params as src
            // Copy source data
            MatrixWorld dst = (MatrixWorld)src.Clone();

            // Apply transformation based on selected type
            ApplyTransformation(dst, transformationType);

            // Store result
            data.StoreProduct(this, dst);
        }
        
        private void ApplyTransformation(MatrixWorld matrix, TransformationType transformation)
        {
            if (transformation == TransformationType.None)
                return;

            int size = matrix.rect.size.x;
            float[] tempArr = new float[matrix.arr.Length];
            System.Array.Copy(matrix.arr, tempArr, matrix.arr.Length);
            ...
            switch (transformation)
            {
                case TransformationType.FlipX:
                    ...
                    for (int z = 0; z < size; z++)
                    {
                        ...
                        {
                            int flippedX = size - 1 - x;
                            matrix.arr[z * size + x] = tempArr[z * size + flippedX];
                        }
                    }
                ...
                case TransformationType.FlipZ:
                    ...
                    for (int z = 0; z < size; z++)
                    {
                        int flippedZ = size - 1 - z;
                        for (int x = 0; x < size; x++)
                        {
                            matrix.arr[z * size + x] = tempArr[flippedZ * size + x];
                        }
                    }
                ...
                case TransformationType.Transpose:
                    ...
                    for (int z = 0; z < size; z++)
                    {
                        for (int x = 0; x < size; x++)
                        {
                            matrix.arr[z * size + x] = tempArr[x * size + z];
                        }
                    }
                ...
                case TransformationType.RotateCW:
                    ...
                    for (int z = 0; z < size; z++)
                    {
                        for (int x = 0; x < size; x++)
                        {
                            matrix.arr[z * size + x] = tempArr[(size - 1 - x) * size + z];
                        }
                    }
                ...
                case TransformationType.RotateCCW:
                    ...
                    for (int z = 0; z < size; z++)
                    {
                        for (int x = 0; x < size; x++)
                        {
                            matrix.arr[z * size + x] = tempArr[x * size + (size - 1 - z)];
                        }
                    }
                ...
                case TransformationType.Rotate180:
                    ...
                    for (int z = 0; z < size; z++)
                    {
                        ...
                        {
                            int flippedX = size - 1 - x;
                            matrix.arr[z * size + x] = tempArr[flippedZ * size + flippedX];
                        }
                    }
    ...
    [System.Serializable]
    [GeneratorMenu(menu = "Map/Modifiers", name = "Symmetrize",
        iconName = "GeneratorIcons/Direction", disengageable = true)]
    public class Symmetrize210 : Generator, IInlet<MatrixWorld>, IOutlet<MatrixWorld>
    {
        public override (string, int) GetCodeFileLine() =>
            GetCodeFileLineBase(); //to get here with right-click on generator
        ...
        [Val("Blend Strength", min = 0, max = 1)]
        ...
        public override void Generate(TileData data, StopToken stop)
        {
            ...
            // Apply symmetry
            ApplySymmetry(dst, symmetryType, blendStrength);

            // Store result
            data.StoreProduct(this, dst);
        }
        
        private void ApplySymmetry(MatrixWorld matrix, SymmetryType symmetryType, float blendStrength)
        {
            if (symmetryType == SymmetryType.None)
                return;

            int size = matrix.rect.size.x;
            int zSize = matrix.rect.size.z;
            float[] tempArr = new float[matrix.arr.Length];
            System.Array.Copy(matrix.arr, tempArr, matrix.arr.Length);
            ...
            switch (symmetryType)
            {
                ...
                {
                    Debug.LogWarning(
                        "It is Recommended to use 0.5 for Blend Strength in this mode.");
                    // Mirror around center point (180 degree rotational symmetry)
                    for (int z = 0; z < size; z++)
                    {
                        for (int x = 0; x < size; x++)
                        {
                            int mirrorX = size - 1 - x;
                            int mirrorZ = zSize - 1 - z;

                            // Average the original value with the mirrored value
                            float originalValue = tempArr[z * size + x];
                            float mirrorValue = tempArr[mirrorZ * size + mirrorX];
                            float blendedValue =
                                BlendValues(originalValue, mirrorValue, blendStrength);

                            matrix.arr[z * size + x] = blendedValue;
                            matrix.arr[mirrorZ * size + mirrorX] = blendedValue;
                        }
                    }
                ...
                case SymmetryType.DiagonalZEqualsX:
                    // Mirror around z=x diagonal line
                    for (int z = 0; z < size; z++)
                    {
                        for (int x = 0; x < size; x++)
                        {
                            // Swap x and z for the mirror point
                            int mirrorX = z;
                            int mirrorZ = x;
                            
                            if (x <= z) continue; // Only process half the matrix
                            
                            float originalValue = tempArr[z * size + x];
                            float mirrorValue = tempArr[mirrorZ * size + mirrorX];
                            float blendedValue = BlendValues(originalValue, mirrorValue, blendStrength);
                ...
                case SymmetryType.DiagonalZEqualsNegativeX:
                    // Mirror around z=-x diagonal (or z=size-1-x in array indices)
                    for (int z = 0; z < size; z++)
                    {
                        for (int x = 0; x < size; x++)
                        {
                            // For z=-x diagonal in array coordinates
                            int mirrorX = size - 1 - z;
                            int mirrorZ = size - 1 - x;
                            
                            if (z + x >= size - 1) continue; // Only process half the matrix
                ...
                case SymmetryType.VerticalMidpoint:
                    // Mirror around horizontal midpoint (x = size/2)
                    for (int z = 0; z < size; z++)
                    {
                        for (int x = 0; x < halfSize; x++)
                        {
                            ...
                            float originalValue = tempArr[z * size + x];
                            float mirrorValue = tempArr[z * size + mirrorX];
                            float blendedValue = BlendValues(originalValue, mirrorValue, blendStrength);
                            
                            matrix.arr[z * size + x] = blendedValue;
                            matrix.arr[z * size + mirrorX] = blendedValue;
                        }
                    }
                    break;
                    
                case SymmetryType.HorizontalMidpoint:
                    // Mirror around vertical midpoint (z = size/2)
                    for (int z = 0; z < halfSize; z++)
                    {
                        int mirrorZ = size - 1 - z;
                        
                        for (int x = 0; x < size; x++)
                        {
                            float originalValue = tempArr[z * size + x];
                            float mirrorValue = tempArr[mirrorZ * size + x];
                            float blendedValue = BlendValues(originalValue, mirrorValue, blendStrength);
                            
                            matrix.arr[z * size + x] = blendedValue;
                            matrix.arr[mirrorZ * size + x] = blendedValue;
                        }
                    }
                    break;
                    
                case SymmetryType.Quadrant:
                    // Mirror first quadrant to all four quadrants
                    for (int z = 0; z < size; z++)
                    {
                        int mirrorZ = z < halfSize ? z : size - 1 - z;
                        
                        for (int x = 0; x < size; x++)
                        {
                            int mirrorX = x < halfSize ? x : size - 1 - x;
                            
                            // Get value from first quadrant (top-left)
                            float sourceValue = tempArr[mirrorZ * size + mirrorX];
                            float originalValue = tempArr[z * size + x];
                            float blendedValue = BlendValues(originalValue, sourceValue, blendStrength);
                ...
                case SymmetryType.Octant:
                    // Mirror first octant to all eight octants (combines diagonals and quadrants)
                    // First determine which octant the point belongs to, then map it back to the first octant
                    for (int z = 0; z < size; z++)
                    {
                        for (int x = 0; x < size; x++)
                        {
                            // Calculate coordinates in first octant
                            bool isBottomHalf = z >= halfSize;
                            bool isRightHalf = x >= halfSize;
                            
                            int sourceZ = isBottomHalf ? size - 1 - z : z;
                            int sourceX = isRightHalf ? size - 1 - x : x;
                            
                            // Ensure we're using the first octant (where x <= z)
                            if (sourceX > sourceZ)
                            {
                                int temp = sourceX;
                                sourceX = sourceZ;
                            ...
                            // Get value from first octant
                            float sourceValue = tempArr[sourceZ * size + sourceX];
                            float originalValue = tempArr[z * size + x];
                            float blendedValue = BlendValues(originalValue, sourceValue, blendStrength);
        ...
        private float BlendValues(float original, float symmetrical, float blendStrength)
        {
            // Linear interpolation between original and symmetrical values
            return original * (1 - blendStrength) + symmetrical * blendStrength;
        }
    }
